...
create table if not exists public.store_billing_subscriptions (
  ...
  constraint store_billing_subscriptions_id_store_key unique (id, store_id)
);
...
create table if not exists public.billing_payments (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint billing_payments_subscription_store_fkey
    foreign key (subscription_id, store_id)
    references public.store_billing_subscriptions(id, store_id)
    on delete restrict
);

create index if not exists billing_payments_subscription_idx
  on public.billing_payments(subscription_id, store_id, due_at desc);
...
create table if not exists public.billing_webhook_events (
  ...
  last_error text,
  constraint billing_webhook_processing_claim_consistency check (
...
create table if not exists public.billing_audit_log (
  ...
  created_at timestamptz not null default now(),
  constraint billing_audit_subscription_store_fkey
    foreign key (subscription_id, store_id)
    references public.store_billing_subscriptions(id, store_id)
    on delete restrict
);
...
create index if not exists billing_audit_subscription_store_created_idx
  on public.billing_audit_log(subscription_id, store_id, created_at desc)
  where subscription_id is not null;

create unique index if not exists billing_audit_provider_event_action_idx
