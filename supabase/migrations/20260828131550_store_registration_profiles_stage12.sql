...
-- Cadastro financeiro separado do cadastro publico da loja. A etapa 12 nao
-- altera public.stores e nao cria trial, cliente, assinatura ou cobranca.
...
-- A auditoria registra apenas estado e nomes dos campos alterados. Nenhum CNPJ,
-- e-mail, telefone ou razao social e duplicado no log.
...
begin
  ...
  -- Trava deliberada da etapa 12: somente o seed sintetico autorizado.
  perform 1
  from public.stores store_row
  where store_row.id = p_store_id
    and store_row.store_name = 'Loja DEV Billing Falhas'
    and store_row.registration_source = 'billing_stage5_seed'
    and store_row.status = 'active'
  for update;

  if not found then
    raise exception 'Loja sintetica autorizada nao encontrada.' using errcode = '42501';
  end if;
...
comment on table public.store_billing_registration_profiles is
  'Cadastro financeiro separado de stores. Na etapa 12 recebe somente o seed sintetico autorizado.';
...
comment on function public.save_store_billing_registration_profile(uuid, uuid, text, text, text, text, uuid) is
  'Persiste cadastro validado somente para Loja DEV Billing Falhas, com idempotencia e sem acionar billing financeiro.';
