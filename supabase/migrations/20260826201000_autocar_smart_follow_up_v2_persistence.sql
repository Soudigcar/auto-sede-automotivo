-- Smart Follow-up V2 persistence for the controlled COPILOT rollout.
-- This migration creates configuration and analytics storage only.
-- It does NOT create a scheduler, cron, outbound WhatsApp sender, webhook executor,
-- or enable the create_follow_up capability.
-- AUTOPILOT is intentionally blocked at database level in this rollout.

create table if not exists public.ai_follow_up_global_settings (
  id text primary key default 'primary' check (id = 'primary'),
  enabled boolean not null default false,
  mode text not null default 'off' check (mode in ('off','copilot')),
...
create table if not exists public.ai_follow_up_store_settings (
  ...
  allowed_end time not null default '19:00',
...
-- Match the existing AUTOCAR Production service-only posture. The browser cannot
-- read or mutate these tables directly; all access goes through authenticated
-- server-side routes using the AUTOCAR service role.
drop policy if exists service_only_deny_client_access on public.ai_follow_up_global_settings;
create policy service_only_deny_client_access on public.ai_follow_up_global_settings
  for all to anon, authenticated using (false) with check (false);

drop policy if exists service_only_deny_client_access on public.ai_follow_up_store_settings;
create policy service_only_deny_client_access on public.ai_follow_up_store_settings
  for all to anon, authenticated using (false) with check (false);

drop policy if exists service_only_deny_client_access on public.ai_follow_up_scenarios;
create policy service_only_deny_client_access on public.ai_follow_up_scenarios
  for all to anon, authenticated using (false) with check (false);

drop policy if exists service_only_deny_client_access on public.ai_follow_up_scenario_steps;
create policy service_only_deny_client_access on public.ai_follow_up_scenario_steps
  for all to anon, authenticated using (false) with check (false);

drop policy if exists service_only_deny_client_access on public.ai_follow_up_performance_events;
create policy service_only_deny_client_access on public.ai_follow_up_performance_events
  for all to anon, authenticated using (false) with check (false);

drop policy if exists service_only_deny_client_access on public.ai_follow_up_config_audit;
create policy service_only_deny_client_access on public.ai_follow_up_config_audit
  for all to anon, authenticated using (false) with check (false);

comment on table public.ai_follow_up_global_settings is 'Master ceiling for Smart Follow-up V2. COPILOT-only rollout; does not enable outbound execution.';
comment on table public.ai_follow_up_store_settings is 'Store preferences constrained by Master. AUTOPILOT is blocked in this rollout.';
comment on table public.ai_follow_up_scenarios is 'Versioned Smart Follow-up journeys. Disabled by default.';
comment on table public.ai_follow_up_scenario_steps is 'Configurable timing steps. No scheduler is created by this migration.';
comment on table public.ai_follow_up_performance_events is 'Append-only attribution/funnel telemetry. Does not send or schedule messages.';
comment on table public.ai_follow_up_config_audit is 'Append-only configuration audit trail.';
