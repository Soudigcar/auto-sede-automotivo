...
const masterMenu = [
  ...
  { label: 'Lojas & Estoque', href: '/master/stores', icon: Store },
    ...
    <aside className="hidden w-72 shrink-0 bg-[#071020] px-6 py-7 text-white lg:block">
      ...
      <nav className="mt-8 space-y-3 text-sm">
        {masterMenu.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={isActive ? 'flex items-center gap-3 rounded-2xl bg-red-600 px-4 py-4 font-bold shadow-lg shadow-red-600/20' : 'flex items-center gap-3 rounded-2xl px-4 py-4 text-zinc-400 hover:bg-white/5 hover:text-white'}>
