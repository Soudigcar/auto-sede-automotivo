...
export default function MasterWhatsappCommerceActions({ conversationId, leadId, baseLeadId, onRefresh, onStatus, compact = false }: {
          ...
          <div className="max-h-[85vh] w-full max-w-2xl overflow-hidden rounded-[24px] border border-zinc-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-200 px-5 py-4">
              <div>
                ...
                <h3 className="mt-1 text-lg font-black text-zinc-950">{mode === 'stock' ? 'Veículos publicados no portal' : mode === 'photos' ? 'Enviar fotos do veículo do portal' : mode === 'transfer' ? 'Transferir lead' : 'Agendar atividade'}</h3>
              </div>
            ...
            {mode === 'transfer' ? (
              ...
              <div className="max-h-[70vh] overflow-auto p-5">
                ...
                {mode === 'stock' ? (
                  <div className="mt-5 grid gap-2 sm:grid-cols-2"><button type="button" disabled={saving} onClick={() => void setVehicleInterest(true)} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 text-xs font-black uppercase text-blue-700 disabled:opacity-50">{saving ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />} Definir como interesse</button><button type="button" disabled={saving} onClick={() => void sendVehiclePhotos()} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-violet-600 px-3 text-xs font-black uppercase text-white disabled:opacity-50">{saving ? <Loader2 size={15} className="animate-spin" /> : <Camera size={15} />} Enviar fotos</button></div>
                ) : (
