...
type CapabilityCounts = {
  ...
  configuration_mode: 'preview_synthetic' | 'production_config' | null;
};
...
export function WhatsappCloudApiPanel({ storeName, storeSlug }: Props) {
  ...
  const configurationAvailable = capabilities?.configuration_available === true;
  ...
  const environmentLabel = syntheticOnly
    ? 'Homologação · execução externa OFF'
    : configurationAvailable
      ? 'Configuração liberada · execução externa OFF'
      : 'Configuração bloqueada · execução externa OFF';
    ...
    <section className="mt-8 space-y-6">
      <header className="premium-card overflow-hidden p-0">
        <div className="bg-gradient-to-br from-[#071020] via-[#10213a] to-[#17345a] p-6 text-white md:p-8">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            ...
            <span className="inline-flex self-start rounded-2xl border border-amber-300/30 bg-amber-300/10 px-4 py-3 text-xs font-black uppercase tracking-wider text-amber-200">{environmentLabel}</span>
          </div>
        </div>
      </header>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <article className="premium-card p-6 md:p-7">
          ...
          {!syntheticOnly && !configurationAvailable ? (
            <p className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm font-bold text-amber-900">A configuração em Production está bloqueada. Nenhuma alteração ou credencial pode ser salva até liberação explícita do gate.</p>
          ) : null}
          ...
          <button disabled={Boolean(busy)||!configurationAvailable} onClick={()=>post('save-draft', form)} className="mt-5 rounded-xl bg-zinc-950 px-4 py-3 text-sm font-black text-white disabled:opacity-50">{busy==='save-draft'?<Loader2 className="inline animate-spin" size={17}/>:null} Salvar configuração</button>
          ...
          <div className="mt-8 border-t border-zinc-200 pt-6">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              <button disabled={Boolean(busy)||!configurationAvailable||!integration?.configured} onClick={()=>post(syntheticOnly ? 'save-synthetic-secrets' : 'save-secrets',form)} className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-sm font-black text-blue-800 disabled:opacity-50">Salvar no Vault</button>
              <button disabled={Boolean(busy)||!configurationAvailable||!hasAllSecrets} onClick={()=>post(syntheticOnly ? 'revoke-synthetic-secrets' : 'revoke-secrets')} className="rounded-xl border border-zinc-200 px-4 py-2.5 text-sm font-black text-zinc-700 disabled:opacity-50">Revogar segredos</button>
            </div>
          </div>
