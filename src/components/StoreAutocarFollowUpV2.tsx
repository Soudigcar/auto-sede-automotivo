  ...
  return <div className="space-y-5">
    ...
    <section className="grid gap-5 xl:grid-cols-[1fr_0.9fr]">
      <div className="premium-card p-5"><div className="flex items-center gap-2"><SlidersHorizontal size={17} className="text-red-600"/><h3 className="text-lg font-black">Configurações do Follow-up</h3></div><div className="mt-4 grid gap-3 sm:grid-cols-2">
        <label className="text-xs font-black">Status do Follow-up<select disabled={!canManage} value={requested.enabled ? 'on' : 'off'} onChange={(e) => change('enabled', e.target.value === 'on')} className="premium-input mt-1.5"><option value="off">DESATIVADO</option><option value="on">ATIVADO</option></select></label>
        <label className="text-xs font-black">Modo do Follow-up<select disabled={!canManage} value={requested.mode} onChange={(e) => change('mode', e.target.value as FollowUpMode)} className="premium-input mt-1.5"><option value="off">OFF</option><option value="copilot">COPILOT</option><option value="autopilot">AUTOPILOT</option></select></label>
