...
const navItems: Array<{ key: SectionKey; label: string; icon: React.ReactNode; helper: string }> = [
  ...
  { key: 'autonomia', label: 'Autonomia', icon: <Gauge size={18} />, helper: 'Controle operacional oficial' },
    ...
    <section className="mt-6 overflow-hidden rounded-[28px] border border-zinc-200 bg-white shadow-sm">
      ...
      <div className="grid xl:grid-cols-[270px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 p-4 md:p-6">
          ...
          {section === 'regras' ? <RulesSection config={config} update={update} slug={slug} /> : null}
          ...
          {section !== 'metodo' && section !== 'teste' && section !== 'autonomia' ? (
...
function RulesSection({ config, update, slug }: SectionProps & { slug: string }) {
  return <SectionShell eyebrow="Operação comercial" title="Regras Comerciais" description="A loja reduz a autonomia da IA e define quando ela deve pedir ajuda. As regras globais de segurança continuam acima destas configurações."><TextArea label="Política de desconto" value={config.discountPolicy} onChange={(v) => update('discountPolicy', v)} placeholder="Ex.: a IA nunca oferece desconto espontaneamente; qualquer desconto precisa de aprovação do gestor." /><TextArea label="Limite de negociação" value={config.negotiationLimit} onChange={(v) => update('negotiationLimit', v)} placeholder="Ex.: pode explicar preço e benefícios, mas não pode alterar valor ou prometer condição final." /><TextArea label="Quando transferir para um humano" value={config.humanHandoffRules} onChange={(v) => update('humanHandoffRules', v)} placeholder="Ex.: intenção clara de compra, contraproposta, reclamação, condição excepcional..." /><div className="md:col-span-2 rounded-2xl border border-zinc-200 bg-zinc-50 p-4"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p className="text-xs font-black text-zinc-900">Smart Follow-up</p><p className="mt-1 max-w-2xl text-xs leading-5 text-zinc-600">Cadência, horários, limites e jornadas são administrados exclusivamente na configuração oficial do Smart Follow-up. Esta Visão Geral não cria uma segunda configuração.</p></div><a href={`/loja/${slug}/autocar/follow-up`} className="inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-xl border border-zinc-300 bg-white px-4 text-xs font-black text-zinc-900 transition hover:bg-zinc-100">Abrir configurações de Follow-up <ChevronRight size={15} /></a></div></div></SectionShell>;
}
...
function AutonomySection() {
  return (
    <div><p className="premium-eyebrow">Controle operacional</p><h3 className="mt-2 text-2xl font-black text-zinc-950">Autonomia</h3><p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-600">A Visão Geral não altera OFF, COPILOT ou AUTOPILOT. O modo operacional real é controlado exclusivamente pelo Modo geral da AUTOCAR, respeitando permissões e regras oficiais.</p><div className="mt-5 rounded-2xl border border-zinc-200 bg-zinc-50 p-4"><p className="text-xs font-black text-zinc-900">Uma única fonte de verdade</p><p className="mt-2 text-xs leading-5 text-zinc-600">Use o controle oficial “Modo geral da AUTOCAR” exibido no portal para consultar ou administrar o modo operacional. Os valores antigos deste rascunho local continuam preservados por compatibilidade, mas não são apresentados como controle operacional.</p></div><div className="mt-5 grid gap-3 md:grid-cols-2"><SafetyItem icon={<BadgeDollarSign size={17} />} title="Desconto" text="Sempre sujeito a aprovação e às regras globais." /><SafetyItem icon={<UsersRound size={17} />} title="Handoff" text="Pode transferir para humano quando a conversa atingir gatilhos definidos." /></div></div>
  );
}
