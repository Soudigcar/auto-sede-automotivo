...
export function EventCreateForm({ onSaved }: { onSaved?: () => void }) {
  ...
  const [form, setForm] = useState({ eventName: '', startDate: '', location: '', sponsorBank: 'Bradesco', liveUrl: '' });
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (error) {
      setMessage('Erro ao cadastrar evento. Rode o SQL de eventos no Supabase.');
      return;
    }

    setMessage('Evento cadastrado com sucesso.');
    setForm({ eventName: '', startDate: '', location: '', sponsorBank: 'Bradesco', liveUrl: '' });
    onSaved?.();
  }
  ...
  return (
    <form onSubmit={save} className="premium-card p-6">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2">
        ...
        <input className="premium-input" type="date" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} />
        <input className="premium-input" placeholder="Local do evento" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} />
