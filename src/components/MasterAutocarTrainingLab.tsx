...
export function MasterAutocarTrainingLab() {
  ...
  async function saveScenario(event: React.FormEvent) {
    ...
    setBusy(true);
    setMessage('Salvando aprendizado oficial...');
    try {
      const access = await token();
      const response = await fetch('/api/master/autocar/training', {
        method: 'POST',
      ...
      setForm(blank);
      await load();
      setMessage(`Aprendizado salvo e indexado em ${environmentLabel(body.environment)}.`);
    } catch (error: any) {
  ...
  async function simulate() {
    ...
    setBusy(true);
    setMessage('Simulando AUTOCAR com Método Venda Mais + aprendizados aprovados...');
    setResult(null);
    try {
      const access = await token();
      const response = await fetch('/api/master/autocar/training', {
        ...
        body: JSON.stringify({ action: 'simulate', customer_input: question })
      });
      const body = await readResponse(response);
      if (!response.ok) throw new Error(body.error || 'Falha na simulação.');
      setResult(body);
      setCorrection(body.response || '');
      setMessage(`Simulação concluída em ${environmentLabel(body.environment)}. Revise a resposta antes de transformar em aprendizado.`);
      await load();
    } catch (error: any) {
  ...
  async function review(evaluation: 'approved' | 'corrected' | 'rejected') {
    ...
    try {
      ...
      const body = await readResponse(response);
      if (!response.ok) throw new Error(body.error || 'Falha ao revisar.');
      setMessage(body.learning
        ? `Resposta revisada e salva como aprendizado em ${environmentLabel(body.environment)}.`
        : `Simulação revisada em ${environmentLabel(body.environment)}.`);
      setResult(null);
      setQuestion('');
      setCorrection('');
      await load();
  ...
  async function archive(id: string) {
    ...
    try {
      ...
      await load();
      setMessage(`Aprendizado arquivado em ${environmentLabel(body.environment)}.`);
    } catch (error: any) {
        ...
        <div className="premium-canvas min-w-0 flex-1 p-4 md:p-7">
          <header className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div>
              ...
              <p className="premium-muted mt-3 max-w-4xl text-sm">Ensine situações reais, teste perguntas de clientes e transforme correções aprovadas em comportamento oficial da AUTOCAR.</p>
            </div>
          ...
          <section className="mt-6 grid gap-5 2xl:grid-cols-2">
            <form onSubmit={saveScenario} className="premium-card p-5 md:p-6">
              <div className="flex items-center gap-2 text-red-600"><Save size={18} /><h2 className="text-xl font-black text-zinc-950">Ensinar</h2></div>
              <p className="mt-2 text-xs leading-5 text-zinc-500">Cadastre como a AUTOCAR deve conduzir uma situação. Aprendizados aprovados entram na busca semântica do simulador.</p>
              <div className="mt-4 grid gap-3">
                <Field label="Pergunta ou situação do cliente"><textarea className="premium-input min-h-24" value={form.situation} onChange={(event) => setForm({ ...form, situation: event.target.value })} placeholder="Ex.: Só tenho R$ 5 mil de entrada. Dá negócio?" /></Field>
                <Field label="Intenção"><input className="premium-input" value={form.intent} onChange={(event) => setForm({ ...form, intent: event.target.value })} placeholder="Ex.: financiamento / entrada" /></Field>
                ...
                <div className="grid gap-3 md:grid-cols-2"><Field label="Restrições — uma por linha"><textarea className="premium-input min-h-20" value={form.restrictions} onChange={(event) => setForm({ ...form, restrictions: event.target.value })} placeholder="Não prometer aprovação\nNão inventar parcela" /></Field><Field label="Tags"><input className="premium-input" value={form.tags} onChange={(event) => setForm({ ...form, tags: event.target.value })} placeholder="financiamento, objeção, entrada" /></Field></div>
                ...
                <div className="grid gap-3 md:grid-cols-2"><Field label="Prioridade"><input className="premium-input" type="number" min="1" max="1000" value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })} /></Field><Field label="Status"><select className="premium-input" value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as 'draft' | 'approved' })}><option value="approved">Aprovado / ativo</option><option value="draft">Rascunho</option></select></Field></div>
              </div>
              <div className="mt-4 flex gap-2"><button disabled={busy || !form.situation.trim() || !form.ideal_response.trim()} className="premium-button-primary flex-1 justify-center"><Save size={16} />{form.scenario_id ? 'Atualizar aprendizado' : 'Salvar aprendizado'}</button>{form.scenario_id ? <button type="button" className="premium-button-secondary" onClick={() => setForm(blank)}>Cancelar edição</button> : null}</div>
            </form>
            ...
            <div className="premium-card p-5 md:p-6">
              ...
              <p className="mt-2 text-xs leading-5 text-zinc-500">Faça a pergunta como um cliente faria. A resposta usa aprendizados aprovados + Método Venda Mais + Biblioteca Global. Nada é enviado ao WhatsApp.</p>
              <textarea className="premium-input mt-4 min-h-28" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="Ex.: Esse carro está muito caro, vi outro mais barato." />
              ...
              {result ? <div className="mt-5 space-y-3">
                ...
                <div className="rounded-2xl border border-zinc-200 p-4"><p className="text-[10px] font-black uppercase tracking-wider text-zinc-400">Por que respondeu assim</p><p className="mt-2 text-xs leading-5 text-zinc-600">{result.reasoning_summary}</p><p className="mt-3 text-[10px] font-black uppercase text-zinc-400">Próxima ação</p><p className="mt-1 text-xs font-bold text-zinc-700">{result.next_action}</p></div>
                ...
                <label className="flex items-center gap-2 text-xs font-bold text-zinc-700"><input type="checkbox" checked={saveLearning} onChange={(event) => setSaveLearning(event.target.checked)} />Salvar resposta aprovada/corrigida como aprendizado oficial</label>
          ...
          <section className="premium-card mt-6 p-5 md:p-6">
            <div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-black">Aprendizados oficiais</h2><p className="mt-1 text-xs text-zinc-500">{data.scenarios.length} cenários cadastrados no ambiente de treinamento.</p></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-black uppercase text-emerald-700">{currentEnvironment}</span></div>
            <div className="mt-4 grid gap-3 xl:grid-cols-2">{data.scenarios.map((item) => <div key={item.id} className="rounded-2xl border border-zinc-200 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap gap-2"><span className={`rounded-full px-2 py-1 text-[9px] font-black uppercase ${item.status === 'approved' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{item.status === 'approved' ? 'Aprovado' : 'Rascunho'}</span>{item.intent ? <span className="rounded-full bg-zinc-100 px-2 py-1 text-[9px] font-black text-zinc-600">{item.intent}</span> : null}</div><p className="mt-3 text-sm font-black text-zinc-900">{item.situation}</p><p className="mt-2 text-xs leading-5 text-zinc-600"><strong>Resposta ideal:</strong> {item.ideal_response}</p>{item.objective ? <p className="mt-2 text-xs text-zinc-500"><strong>Objetivo:</strong> {item.objective}</p> : null}</div><CheckCircle2 size={18} className={item.status === 'approved' ? 'text-emerald-600' : 'text-zinc-300'} /></div><div className="mt-3 flex gap-2"><button type="button" onClick={() => editScenario(item)} className="rounded-lg border border-zinc-200 px-3 py-2 text-[10px] font-black text-zinc-700"><PencilLine size={12} className="mr-1 inline" />Editar</button><button type="button" onClick={() => void archive(item.id)} className="rounded-lg border border-zinc-200 px-3 py-2 text-[10px] font-black text-zinc-500"><Archive size={12} className="mr-1 inline" />Arquivar</button></div></div>)}{!data.scenarios.length ? <div className="rounded-2xl border border-dashed border-zinc-300 p-7 text-center text-sm font-bold text-zinc-400">Nenhum aprendizado ainda. Cadastre o primeiro cenário acima.</div> : null}</div>
          </section>
