            ...
            {editingId === item.id ? (
              ...
              <>
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                  <strong className={item.movement_type === 'expense' ? 'text-red-600' : 'text-emerald-600'}>{item.movement_type === 'expense' ? 'Saída' : 'Entrada'} - {item.sponsor_bank || item.supplier_name || 'Lançamento'} - {money(item.amount)}</strong>
                  <span className="text-xs font-black uppercase tracking-wide text-zinc-400">{item.payment_date || 'sem data'}</span>
                </div>
                <p className="mt-1 text-sm text-zinc-500">{item.event_name} | {item.category} | fornecedor: {item.supplier_name || '-'} | desconto: {money(item.discount)}</p>
                <div className="mt-4 flex flex-wrap gap-2"><button className="premium-button-secondary text-xs" type="button" onClick={() => startEdit(item)}><Pencil size={14} /> Editar</button><button className="premium-button-secondary text-xs" type="button" onClick={() => removeItem(item)}><Trash2 size={14} /> Excluir</button></div>
              </>
            )}
          </div>
        ))}
...
function Mini({ label, value, tone }: { label: string; value: string; tone: string }) {
  return <div className="rounded-2xl border border-zinc-100 bg-zinc-50 p-3"><p className="text-xs font-bold text-zinc-400">{label}</p><strong className={`mt-1 block text-sm ${tone}`}>{value}</strong></div>;
}
