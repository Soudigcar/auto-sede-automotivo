...
import { Archive, BookOpen, Bot, CheckCircle2, FileText, LibraryBig, Loader2, RefreshCw, Search, ShieldCheck, Sparkles, Store, Upload } from 'lucide-react';
import { MasterSidebar } from '@/components/MasterSidebar';
import { createClient } from '@/lib/supabase';

type Mode = 'off' | 'copilot' | 'autopilot';
type Tab = 'overview' | 'method' | 'library' | 'stores' | 'rules' | 'tests' | 'monitoring';
type StoreRow = { id:string; store_name:string; slug:string|null; status:string|null; portal_enabled:boolean|null; autocar:{mode:Mode;status:string;updated_at:string}|null };
...
type Payload = { environment:string; stores:StoreRow[]; documents:Doc[]; summary:{total_stores:number;enabled:number;copilot:number;autopilot:number;global_documents:number} };
...
export function MasterAutocarCenter(){
  ...
  return <main className="premium-page"><section className="premium-shell flex min-h-screen"><MasterSidebar active="/master/autocar"/><div className="premium-canvas min-w-0 flex-1 p-4 md:p-7">
    ...
    {tab==='stores'?<section className="mt-6 premium-card p-5"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="text-xl font-black">Liberação por loja</h2><p className="mt-1 text-xs text-zinc-500">OFF desliga; COPILOT sugere sem enviar; AUTOPILOT fica preparado para a futura automação controlada.</p></div><div className="relative"><Search size={15} className="absolute left-3 top-3 text-zinc-400"/><input className="premium-input pl-9" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar loja"/></div></div><div className="mt-5 space-y-2">{stores.map(store=><div key={store.id} className="flex flex-col gap-3 rounded-2xl border border-zinc-200 p-4 md:flex-row md:items-center md:justify-between"><div><p className="font-black text-zinc-900">{store.store_name}</p><p className="mt-1 text-[11px] font-bold text-zinc-400">{store.slug||'sem slug'} · atual: {(store.autocar?.mode||'off').toUpperCase()}</p></div><div className="flex flex-wrap gap-2">{(['off','copilot','autopilot'] as Mode[]).map(mode=><button key={mode} disabled={busy} onClick={()=>void setMode(store,mode)} className={`rounded-xl px-3 py-2 text-[10px] font-black uppercase ${((store.autocar?.mode||'off')===mode)?'bg-red-600 text-white':'border border-zinc-200 bg-white text-zinc-600'}`}>{mode}</button>)}</div></div>)}</div></section>:null}

    {['rules','tests','monitoring'].includes(tab)?<section className="premium-card mt-6 p-7"><Bot size={28} className="text-red-600"/><h2 className="mt-4 text-2xl font-black">{tab==='rules'?'Regras Globais':tab==='tests'?'Laboratório de Testes':'Monitoramento AUTOCAR'}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-600">{tab==='rules'?'Aqui ficarão os limites globais que nenhuma loja poderá ultrapassar. As hard policies do código continuam tendo prioridade.':tab==='tests'?'Área preparada para simular atendimentos com Método Venda Mais + contexto de uma loja antes de liberar automação.':'Área preparada para acompanhar runs, custo, handoffs, aprovações e qualidade por loja.'}</p><span className="mt-4 inline-flex rounded-full bg-amber-50 px-3 py-1 text-[10px] font-black uppercase text-amber-700">Preparado para próxima fase</span></section>:null}
  </div></section></main>;
}
