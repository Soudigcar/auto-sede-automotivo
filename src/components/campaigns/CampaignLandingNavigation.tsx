...
export function CampaignLandingNavigation({
  ...
  return (
    <nav
      ...
      className={`sticky top-0 z-[85] w-full border-b border-white/10 bg-[#071020]/95 text-white shadow-xl backdrop-blur-xl ${preview ? 'relative' : ''}`}
    >
        ...
        <div className="flex w-full max-w-xl items-center justify-center divide-x divide-white/20 overflow-hidden rounded-full border border-white/15 bg-white/5 p-1">
          {items.map((item) => (
            <button
              ...
              className="group relative min-h-10 flex-1 px-2 text-[10px] font-black tracking-[0.12em] text-white transition hover:bg-white/10 sm:px-5 sm:text-xs sm:tracking-[0.18em]"
            >
