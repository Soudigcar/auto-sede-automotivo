...
type CampaignSimulatorCardProps = { campaign: any; vehicles: any[]; primaryColor?: string; onOpen: () => void; cardRadius?: number; buttonRadius?: number; buttonTextColor?: string; };
type CampaignFinanceSimulatorModalProps = { campaign: any; eventInfo?: any; vehicles: any[]; open: boolean; onClose: () => void; initialVehicleId?: string; mode?: SimulatorMode; primaryColor?: string; slug?: string; };
...
function maskPhone(value: string) { const clean = digits(value).slice(0, 11); return clean.length <= 10 ? clean.replace(/^(\d{2})(\d)/, '($1) $2').replace(/(\d{4})(\d)/, '$1-$2') : clean.replace(/^(\d{2})(\d)/, '($1) $2').replace(/(\d{5})(\d)/, '$1-$2'); }
...
const inputClass = 'h-9 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm font-medium text-slate-900 outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200';
function SectionTitle({ icon: Icon, title, description }: { icon: any; title: string; description: string }) { return <div className="flex items-start gap-2.5"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-slate-200 text-slate-600"><Icon size={17} /></span><div><h3 className="text-sm font-black text-slate-900">{title}</h3><p className="text-[11px] leading-4 text-slate-500">{description}</p></div></div>; }
...
export function CampaignFinanceSimulatorModal({ campaign, eventInfo, vehicles, open, onClose, initialVehicleId = '', mode = 'live', primaryColor, slug = '' }: CampaignFinanceSimulatorModalProps) {
  ...
  function setDownPaymentPercent(percent: number) { if (selectedVehicle?.price) setForm({ ...form, down_payment: String(Math.round(Number(selectedVehicle.price) * percent / 100)) }); }
  ...
  const whatsappMessage = encodeURIComponent(`Olá, sou ${form.name.trim()}. Enviei uma simulação para ${[selectedVehicle?.brand, selectedVehicle?.model, selectedVehicle?.year].filter(Boolean).join(' ')} com entrada de ${money(simulation.downPayment)} em ${simulation.installments}x e desejo antecipar meu atendimento.`);

  if (!open) return null;
  return <div className="fixed inset-0 z-[350] flex items-center justify-center bg-slate-950/75 p-3 backdrop-blur-sm">
    <div className="flex max-h-[92vh] w-full max-w-[1240px] flex-col overflow-hidden rounded-[24px] bg-white shadow-2xl">
      ...
      <form onSubmit={submit} className="grid min-h-0 flex-1 gap-4 overflow-y-auto px-5 pb-5 sm:px-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-3">
          ...
          <section className="rounded-xl border border-slate-200 p-3"><SectionTitle icon={UserRound} title="Seus dados" description="Informações para realizar sua simulação." /><div className="mt-3 grid gap-2.5 sm:grid-cols-2"><label className="text-[11px] font-bold text-slate-700">Nome completo<input className={`${inputClass} mt-1`} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></label><label className="text-[11px] font-bold text-slate-700">Telefone<input className={`${inputClass} mt-1`} value={form.phone} onChange={(e) => setForm({ ...form, phone: maskPhone(e.target.value) })} required /></label><label className="text-[11px] font-bold text-slate-700">CPF<input className={`${inputClass} mt-1`} value={form.cpf} onChange={(e) => setForm({ ...form, cpf: maskCpf(e.target.value) })} required /></label><label className="text-[11px] font-bold text-slate-700">Data de nascimento<input type="date" min="1900-01-01" max={new Date().toISOString().slice(0, 10)} className={`${inputClass} mt-1`} value={form.birth_date} onChange={(e) => setForm({ ...form, birth_date: e.target.value })} required /></label><label className="text-[11px] font-bold text-slate-700 sm:col-span-2">E-mail<input type="email" className={`${inputClass} mt-1`} value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required /></label></div></section>

          <section className="rounded-xl border border-slate-200 p-3"><SectionTitle icon={CarFront} title="Escolha do veículo" description="Selecione o veículo que deseja financiar." /><div className="mt-3 grid items-end gap-3 md:grid-cols-[1fr_180px]"><label className="text-[11px] font-bold text-slate-700">Veículo<select className={`${inputClass} mt-1`} value={form.vehicle_id} onChange={(e) => setForm({ ...form, vehicle_id: e.target.value, down_payment: '' })} required><option value="">Selecione o veículo</option>{vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.brand} {vehicle.model} {vehicle.year || ''}</option>)}</select></label><div><p className="text-[11px] font-bold text-slate-700">Preço do veículo</p><strong className="mt-1 block text-lg font-black">{money(selectedVehicle?.price || 0)}</strong></div></div></section>

          <section className="rounded-xl border border-slate-200 p-3"><SectionTitle icon={Calculator} title="Condições da simulação" description="Informe a entrada e escolha o prazo." /><div className="mt-3 grid gap-3 md:grid-cols-2"><div className="rounded-lg border border-slate-200 p-3"><label className="text-[11px] font-bold text-slate-700">Entrada<input type="number" min="0" step="0.01" max={selectedVehicle?.price || undefined} className={`${inputClass} mt-1`} value={form.down_payment} onChange={(e) => setForm({ ...form, down_payment: e.target.value })} required /></label><div className="mt-2 flex items-center gap-2"><span className="text-[10px] text-slate-500">Percentual:</span>{[20,30,40].map((percent) => <button key={percent} type="button" onClick={() => setDownPaymentPercent(percent)} className="h-7 rounded-md border px-3 text-[11px] font-bold" style={{ borderColor: Number(form.down_payment) === Math.round(Number(selectedVehicle?.price || 0) * percent / 100) ? primary : '#CBD5E1', color: Number(form.down_payment) === Math.round(Number(selectedVehicle?.price || 0) * percent / 100) ? primary : '#475569' }}>{percent}%</button>)}</div></div><div className="rounded-lg border border-slate-200 p-3"><p className="text-[11px] font-bold text-slate-700">Prazo</p><div className="mt-2 grid grid-cols-5 gap-1.5">{campaignInstallmentOptions.map((value) => <button key={value} type="button" onClick={() => setForm({ ...form, installments: String(value) })} className="h-8 rounded-md border text-[11px] font-bold" style={{ borderColor: String(value) === form.installments ? primary : '#CBD5E1', color: String(value) === form.installments ? primary : '#475569' }}>{value}x</button>)}</div><p className="mt-2 flex items-center gap-1.5 border-t border-slate-200 pt-2 text-[10px] text-slate-500"><Info size={12} /> Prazos maiores podem elevar os juros.</p></div></div></section>

          <label className="flex items-start gap-2.5 rounded-xl border border-slate-200 p-3 text-[11px] text-slate-600"><input type="checkbox" className="mt-0.5 h-4 w-4 accent-red-600" checked={form.consent} onChange={(e) => setForm({ ...form, consent: e.target.checked })} required /><span><strong className="block text-slate-800">Autorizo o contato comercial da Auto Sede e lojas participantes.</strong><span className="text-slate-500">Contato por telefone, e-mail ou WhatsApp.</span></span></label>
          <p className="flex items-start gap-2 px-2 text-[10px] text-slate-500"><LockKeyhole size={13} /><span><strong className="text-slate-700">Seus dados serão usados apenas para retornar a proposta.</strong></span></p>
        </div>

        <aside className="self-start rounded-xl bg-slate-950 p-4 text-white xl:sticky xl:top-0"><h3 className="text-base font-black">Resumo da simulação</h3>{selectedVehicle ? <div className="mt-3"><CampaignVehicleGallery vehicle={selectedVehicle} compact /><h4 className="mt-3 text-xl font-black">{selectedVehicle.brand} {selectedVehicle.model}</h4><p className="text-xs text-slate-400">{[selectedVehicle.year, selectedVehicle.fuel, selectedVehicle.transmission].filter(Boolean).join(' • ')}</p></div> : <p className="mt-3 text-xs text-slate-400">Selecione um veículo para visualizar o resumo.</p>}<div className="mt-3 space-y-2 text-xs"><p className="flex justify-between border-b border-white/10 pb-2"><span className="text-slate-400">Preço</span><strong>{money(simulation.vehiclePrice)}</strong></p><p className="flex justify-between border-b border-white/10 pb-2"><span className="text-slate-400">Entrada</span><strong>{hasDownPayment ? money(simulation.downPayment) : '—'}</strong></p><p className="flex justify-between"><span className="text-slate-400">Financiado</span><strong>{hasDownPayment ? money(simulation.financedAmount) : '—'}</strong></p></div><div className="mt-3 rounded-xl bg-white/10 p-3"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Parcela estimada</p>{hasDownPayment ? <><strong className="mt-1.5 block text-3xl font-black">{money(simulation.estimatedInstallment)} <span className="text-xs text-slate-400">/mês</span></strong><p className="text-[10px] text-slate-400">em {simulation.installments}x • taxa referencial</p><div className="mt-2 space-y-1 border-t border-white/10 pt-2 text-[10px]"><p className="flex justify-between"><span className="text-slate-400">Total das parcelas</span><strong>{money(simulation.totalInstallments)}</strong></p><p className="flex justify-between"><span className="text-slate-400">Total com entrada</span><strong>{money(simulation.totalWithDownPayment)}</strong></p></div></> : <p className="mt-1.5 text-xs font-bold">Informe o valor da entrada.</p>}</div><button type="submit" disabled={sending || !selectedVehicle || !hasDownPayment} className="mt-3 h-11 w-full rounded-xl text-sm font-black text-white disabled:opacity-50" style={{ backgroundColor: primary }}>{sending ? 'Enviando...' : mode === 'preview' ? 'Testar simulação' : 'SIMULAR FINANCIAMENTO'}</button><p className="mt-2 text-[9px] leading-4 text-slate-400">Estimativa inicial. Taxa, CET e aprovação final dependem da instituição financeira.</p>{message ? <p className="mt-2 rounded-lg bg-red-500/15 p-2 text-[10px] font-bold text-red-200">{message}</p> : null}</aside>
      </form>}
    </div>
  </div>;
}