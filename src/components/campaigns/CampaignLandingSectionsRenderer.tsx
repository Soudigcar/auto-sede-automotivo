...
function blockStyle(block: LandingSectionBlock): React.CSSProperties {
  return {
    color: block.color,
...
function renderBlock(block: LandingSectionBlock, props: Props) {
  ...
  if (block.type === 'icon') return <article className="border p-6" style={blockStyle(block)}><div className="text-3xl" aria-hidden>{block.icon || '★'}</div><h3 className="mt-3 text-xl font-black">{block.title}</h3>{block.text ? <p className="mt-2 text-sm leading-6 opacity-70">{block.text}</p> : null}</article>;
...
function ContentSection({ section, props }: { section: LandingSection; props: Props }) {
  ...
  const columns = Math.max(1, Math.min(4, section.columns || 3));
  return <div className="mx-auto max-w-6xl px-6" style={{ color: section.textColor }}>
    <div className="space-y-5">{full.map((block) => <div key={block.id}>{renderBlock(block, props)}</div>)}</div>
    {grid.length ? <div className="mt-8 grid gap-4" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>{grid.map((block) => <div key={block.id}>{renderBlock(block, props)}</div>)}</div> : null}
  </div>;
}
...
export function CampaignLandingSectionsRenderer(props: Props) {
  return <>
    {props.draft.sections.map((section) => {
      ...
      if (section.type === 'vehicles') {
        return <section key={section.id} id="landing-vehicles" data-section-id={section.id} className={`relative ${active ? 'outline outline-2 outline-fuchsia-500 outline-offset-[-2px]' : ''}`} style={common} onClick={(event) => { if (props.editor) { event.stopPropagation(); props.onSelectSection?.(section.id); } }}>
          {props.editor ? <div className="pointer-events-none absolute right-4 top-4 z-30 rounded-full bg-fuchsia-600 px-3 py-1.5 text-[10px] font-black text-white">EDITAR SEÇÃO</div> : null}
          <CampaignVehicleDiscovery vehicles={props.vehicles} primaryColor={props.draft.primaryColor} onOpenSimulator={(vehicleId) => props.onOpenSimulator(vehicleId)} settings={section.vehicleSettings} embedded />
        </section>;
      }
      return <section key={section.id} data-section-id={section.id} className={`relative ${active ? 'outline outline-2 outline-fuchsia-500 outline-offset-[-2px]' : ''}`} style={common} onClick={(event) => { if (props.editor) { event.stopPropagation(); props.onSelectSection?.(section.id); } }}>
        {props.editor ? <div className="pointer-events-none absolute right-4 top-4 z-30 rounded-full bg-fuchsia-600 px-3 py-1.5 text-[10px] font-black text-white">EDITAR SEÇÃO</div> : null}
