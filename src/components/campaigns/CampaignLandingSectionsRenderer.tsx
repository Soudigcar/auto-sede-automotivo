...
type Props = { draft: LandingDraftV3; vehicles: any[]; campaign: any; editor?: boolean; previewDevice?: Device; selectedSectionId?: string; onSelectSection?: (id: string) => void; onOpenSimulator: (vehicleId?: string) => void };
...
function renderBlock(block: LandingSectionBlock, props: Props) {
  ...
  if (block.type === 'icon') return <article className="min-w-0 border p-6" style={blockStyle(block)}>{block.image ? <img src={block.image} alt={block.alt || ''} className="h-14 w-14 rounded-xl object-cover" /> : <div className="text-3xl" aria-hidden>{block.icon || '★'}</div>}<h3 className="mt-3 break-words text-xl font-black">{block.title}</h3>{block.text ? <p className="mt-2 break-words text-sm leading-6 opacity-70">{block.text}</p> : null}</article>;
  if (block.type === 'button') return <button type="button" onClick={() => runAction(block, props)} className="min-h-12 max-w-full break-words px-6 py-3 text-sm font-black shadow-sm" style={blockStyle(block)}>{block.label || 'Saiba mais'}</button>;
  return <article className="min-w-0 border p-6" style={blockStyle(block)}><h3 className="break-words text-xl font-black">{block.title}</h3>{block.text ? <p className="mt-2 break-words text-sm leading-6 opacity-70">{block.text}</p> : null}</article>;
}
function gridClass(columns:number, previewDevice?:Device) {
  ...
  if(columns<=1)return 'grid-cols-1'; if(columns===2)return 'grid-cols-1 sm:grid-cols-2'; if(columns===3)return 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'; return 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4';
}
function ContentSection({ section, props }: { section: LandingSection; props: Props }) {
  ...
  return <div className={`mx-auto min-w-0 max-w-6xl ${horizontalPadding}`} style={{color:section.textColor}}><div className="min-w-0 space-y-5">{full.map((block)=><div className="min-w-0" key={block.id}>{renderBlock(block,props)}</div>)}</div>{grid.length?<div className={`mt-8 grid min-w-0 gap-4 ${gridClass(section.columns||3, props.previewDevice)}`}>{grid.map((block)=><div className="min-w-0" key={block.id}>{renderBlock(block,props)}</div>)}</div>:null}</div>;
}
export function CampaignLandingSectionsRenderer(props:Props) {
  ...
  return <>{props.draft.sections.map((section)=>{ if(!section.visible)return null; const active=Boolean(props.editor&&props.selectedSectionId===section.id); const common={backgroundColor:section.backgroundColor,paddingTop:section.paddingY,paddingBottom:section.paddingY,scrollMarginTop:72}; if(section.type==='vehicles')return <section key={section.id} id="landing-vehicles" data-section-id={section.id} className={`relative min-w-0 overflow-hidden ${active?'outline outline-2 outline-fuchsia-500 outline-offset-[-2px]':''}`} style={common} onClick={(event)=>{if(props.editor){event.stopPropagation();props.onSelectSection?.(section.id);}}}>{props.editor?<div className="pointer-events-none absolute right-3 top-3 z-30 rounded-full bg-fuchsia-600 px-3 py-1.5 text-[10px] font-black text-white">EDITAR SEÇÃO</div>:null}<CampaignVehicleDiscovery vehicles={props.vehicles} primaryColor={props.draft.primaryColor} onOpenSimulator={(vehicleId)=>props.onOpenSimulator(vehicleId)} settings={section.vehicleSettings} embedded layoutMode={vehicleLayout} /></section>; return <section key={section.id} data-section-id={section.id} className={`relative min-w-0 overflow-hidden ${active?'outline outline-2 outline-fuchsia-500 outline-offset-[-2px]':''}`} style={common} onClick={(event)=>{if(props.editor){event.stopPropagation();props.onSelectSection?.(section.id);}}}>{props.editor?<div className="pointer-events-none absolute right-3 top-3 z-30 rounded-full bg-fuchsia-600 px-3 py-1.5 text-[10px] font-black text-white">EDITAR SEÇÃO</div>:null}<ContentSection section={section} props={props}/></section>;})}</>;
}
