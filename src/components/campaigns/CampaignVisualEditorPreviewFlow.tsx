...
function rectBox(element: HTMLElement, heroRect: DOMRect, fallback: Box): Box {
  const width = clamp((element.getBoundingClientRect().width / heroRect.width) * 100, 4, 100);
...
export function CampaignVisualEditorPreviewFlow(props: Props) {
  ...
  function visualStyle(key: ContentKey, flowTarget?: ResponsiveTarget): React.CSSProperties {
    ...
    return {
      ...
      letterSpacing: visual.letterSpacing * scale,
      borderRadius: visual.radius,
      padding: `${visual.paddingY * scale}px ${visual.paddingX * scale}px`,
  ...
  function renderTextElement(key: ContentKey, style: React.CSSProperties) {
    ...
    if (key === 'primaryButton' || key === 'secondaryButton') {
      return <button type="button" tabIndex={-1} style={style} onClick={() => props.clientView && runButtonAction(key)}>{text}</button>;
    }
  ...
  function flowContent(key: ContentKey, targetDevice: ResponsiveTarget) {
    ...
    return <div data-editor-element={`content-${key}`} data-flow-box={`content-${key}`} className={`relative ${active ? 'z-40 outline outline-2 outline-fuchsia-400 outline-offset-4' : ''}`} onClick={(event) => { if (props.clientView) return; event.stopPropagation(); props.onSelectContent(key); }} onDoubleClick={(event) => { if (props.clientView) return; event.stopPropagation(); props.onSelectContent(key); window.setTimeout(() => document.getElementById(`text-${key}`)?.focus(), 20); }}>
      {renderTextElement(key, visualStyle(key, targetDevice))}
      {flowSelectionControls('content', key, box.locked)}
    </div>;
  }

  function renderAutoFlow(targetDevice: ResponsiveTarget) {
    const mediaWidth = clamp(layout.backgroundScale, 15, 100);
    const horizontal = layout.backgroundX < 35 ? 'flex-start' : layout.backgroundX > 65 ? 'flex-end' : 'center';
    ...
    return <section ref={setHeroRef} className={`relative overflow-hidden ${!props.clientView && props.layer === 'background' ? 'outline outline-2 outline-cyan-400' : ''}`} style={{ backgroundColor: draft.secondaryColor }} onClick={(event) => { if (!props.clientView && !(event.target as HTMLElement).closest('[data-editor-element]')) props.onSelect('background'); }} onDoubleClick={(event) => { if (!props.clientView && !(event.target as HTMLElement).closest('[data-editor-element]')) props.onBackgroundDoubleClick(); }}>
      <div className={`relative z-10 mx-auto flex w-full flex-col ${targetDevice === 'mobile' ? 'gap-5 px-5 py-7' : 'gap-7 px-8 py-9'}`} style={{ maxWidth: targetDevice === 'mobile' ? 520 : 900 }}>
        ...
        {props.heroSource ? <div className="relative flex min-h-44 w-full items-center overflow-hidden rounded-[28px]" style={{ justifyContent: horizontal, background: `${draft.secondaryColor}22` }}><img src={props.heroSource} alt="Capa" draggable={false} className="pointer-events-none h-auto max-h-[720px] max-w-full select-none object-contain" style={{ width: `${mediaWidth}%` }} />{draft.overlay > 0 ? <div className="pointer-events-none absolute inset-0" style={{ background: `rgba(7,16,32,${draft.overlay / 100})` }} /> : null}{linkedBackground && !props.clientView ? <span className="absolute bottom-3 right-3 rounded-full bg-black/70 px-3 py-1.5 text-[9px] font-black text-white">FUNDO DO DESKTOP</span> : null}</div> : null}
  ...
  const hero = autoFlow && target ? renderAutoFlow(target) : <section ref={setHeroRef} className={`relative overflow-hidden ${!props.clientView && props.layer === 'background' ? 'outline outline-2 outline-cyan-400' : ''}`} style={{ minHeight: layout.heroHeight, backgroundColor: draft.secondaryColor, touchAction: 'none' }} onClick={(event) => { if (!props.clientView && !(event.target as HTMLElement).closest('[data-editor-element]')) props.onSelect('background'); }} onPointerDown={props.onStartBackground} onWheel={props.onWheel} onDoubleClick={(event) => { if (!props.clientView && !(event.target as HTMLElement).closest('[data-editor-element]')) props.onBackgroundDoubleClick(); }}>{props.heroSource ? <img src={props.heroSource} alt="Capa" draggable={false} className="pointer-events-none absolute h-auto max-w-none select-none" style={{ left: `${layout.backgroundX}%`, top: `${layout.backgroundY}%`, width: `${layout.backgroundScale}%`, transform: 'translate(-50%, -50%)' }} /> : null}{props.heroSource && draft.overlay > 0 ? <div className="pointer-events-none absolute inset-0" style={{ background: `rgba(7,16,32,${draft.overlay / 100})` }} /> : null}{absoluteBox('header', <div className="flex items-center gap-3" style={{ color: draft.content.title.color }}>{draft.showHeaderLabel ? <strong>{draft.headerLabel}</strong> : null}<img src={draft.headerLogo || defaultHeader} alt="Apoio" draggable={false} className="pointer-events-none w-full object-contain" /></div>)}{absoluteBox('logo', draft.eventLogo || props.campaign?.logo_url ? <img src={draft.eventLogo || props.campaign?.logo_url} alt="Logo" draggable={false} className="pointer-events-none w-full object-contain" /> : <div className="rounded-xl border border-dashed border-white/50 p-4 text-white">Adicionar logo</div>)}{contentKeys.map((key) => <span key={key}>{absoluteContent(key)}</span>)}{absoluteBox('simulator', <CampaignFinanceSimulatorInline campaign={props.campaign} eventInfo={props.eventInfo} vehicles={props.vehicles} primaryColor={draft.primaryColor} cardRadius={draft.cardRadius} stacked={props.device !== 'desktop'} />, true)}</section>;

  const fallbackVehicles = useMemo(() => Array.from({ length: props.device === 'mobile' ? 3 : props.device === 'tablet' ? 4 : 8 }).map((_, index) => ({ id: `preview-${index}`, brand: 'Veículo', model: `disponível ${index + 1}`, version: 'Consulte condições', store_name: 'Loja participante' })), [props.device]);
