...
export function CampaignVisualEditorPreviewResponsive(props: Props) {
  ...
  const [panelMode, setPanelMode] = useState<'menu' | 'sections'>('sections');
  ...
  const editorPanel = !props.clientView && panelOpen && typeof document !== 'undefined' ? createPortal(
    <div className="fixed bottom-0 right-0 top-[76px] z-[180] flex w-[380px] flex-col border-l bg-white shadow-2xl">
      <div className="flex items-center justify-between border-b px-4 py-3">
        <div className="flex items-center gap-2"><Layers3 size={17}/><div><strong className="text-sm">Construtor da landing</strong><p className="text-[9px] font-bold uppercase tracking-[.12em] text-fuchsia-600">Layout v3 controlado</p></div></div>
      ...
      <div className="grid grid-cols-2 gap-2 border-b p-3">
        <button type="button" onClick={() => setPanelMode('menu')} className={`rounded-xl px-3 py-2 text-[10px] font-black ${panelMode === 'menu' ? 'bg-indigo-600 text-white' : 'bg-zinc-100 text-zinc-700'}`}><Menu size={13} className="inline"/> MENU</button>
        <button type="button" onClick={() => setPanelMode('sections')} className={`rounded-xl px-3 py-2 text-[10px] font-black ${panelMode === 'sections' ? 'bg-fuchsia-600 text-white' : 'bg-zinc-100 text-zinc-700'}`}><Layers3 size={13} className="inline"/> SEÇÕES</button>
      </div>
      ...
      <div className="border-t bg-emerald-50 p-3 text-[10px] font-bold text-emerald-800">As alterações agora atualizam o draft principal do editor. “Salvar rascunho” e “Publicar” continuam sob seu controle.</div>
    </div>, document.body
  ) : null;
  ...
  return <div id="editor-inicio" className="relative min-w-0 overflow-hidden bg-slate-50">
    ...
    {!props.clientView ? <div className="flex min-w-0 items-center justify-between gap-3 border-b border-indigo-200 bg-indigo-50 px-4 py-2.5">
      <div className="min-w-0"><p className="text-[10px] font-black text-indigo-900">Landing por telas independentes</p><p className="truncate text-[9px] font-semibold text-indigo-700">Edite o menu ou as seções sem alterar a navegação pública por rolagem.</p></div>
      <div className="flex shrink-0 gap-2"><button type="button" onClick={() => { setPanelMode('menu'); setPanelOpen(true); }} className="rounded-xl bg-indigo-600 px-3 py-2 text-[9px] font-black text-white"><Menu size={13} className="inline"/> Editar menu</button><button type="button" onClick={() => { setPanelMode('sections'); setPanelOpen(true); }} className="rounded-xl bg-fuchsia-600 px-3 py-2 text-[9px] font-black text-white"><Layers3 size={13} className="inline"/> Editar seções</button></div>
    </div> : null}
    ...
    {activeView === 'home' ? <>
      ...
      <CampaignLandingSectionsRenderer draft={draftV3} vehicles={props.vehicles} campaign={props.campaign} editor={!props.clientView} previewDevice={props.device} selectedSectionId={selectedSectionId} onSelectSection={chooseSection} onOpenSimulator={() => setActiveView('simulation')} view="home" />
    </> : null}

    {activeView === 'vehicles' ? <CampaignLandingSectionsRenderer draft={draftV3} vehicles={props.vehicles} campaign={props.campaign} editor={!props.clientView} previewDevice={props.device} selectedSectionId={selectedSectionId} onSelectSection={chooseSection} onOpenSimulator={() => setActiveView('simulation')} view="vehicles" /> : null}

    {activeView === 'simulation' ? <section className="min-h-[720px] bg-slate-100 px-4 py-10 sm:px-6"><div className="mx-auto max-w-6xl"><CampaignFinanceSimulatorInline campaign={props.campaign} eventInfo={props.eventInfo} vehicles={props.vehicles} primaryColor={draftV3.primaryColor} cardRadius={draftV3.cardRadius} backgroundColor={draftV3.simulatorBackground} summaryBackgroundColor={draftV3.simulatorSummaryBackground} /></div></section> : null}
