...
import { clamp, contentKeys, contentNames, defaultHeader, textFor } from './CampaignVisualEditorModel';
...
export function CampaignVisualEditorInspector(p: any) {
  ...
  return <aside className="overflow-y-auto bg-white p-4">
    ...
    {layer === 'simulator' ? <><div className="mt-4 rounded-xl bg-emerald-50 p-3 text-xs font-bold text-emerald-800">Use a alça superior para mover sem bloquear os campos.</div><BoxPanel box={layout.simulator} min={44} onChange={(value: any) => p.patchBox('simulator', value)} /><Num label="Curvatura" value={draft.cardRadius} min={0} max={80} suffix="px" onChange={(cardRadius: number) => p.commit({ ...draft, cardRadius })} /></> : null}
