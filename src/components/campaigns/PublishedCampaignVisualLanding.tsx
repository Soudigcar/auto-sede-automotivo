...
export function PublishedCampaignVisualLanding({ campaign, eventInfo, vehicles, stores, slug }: Props) {
  ...
  const draft = useMemo(() => ensureResponsive(safe(campaign?.published_layout, campaign)), [campaign]);
  ...
  return <main id="landing-inicio" className="min-h-screen bg-slate-50 text-slate-950">
    ...
    <style jsx global>{`
      ...
      .published-campaign-flow > div > footer { display: none; }
      .published-campaign-flow > div > section:nth-of-type(2) { background-color: ${draft.advantagesBackground} !important; }
      #landing-inicio #veiculos { background-color: ${draft.vehiclesBackground} !important; }
    `}</style>
