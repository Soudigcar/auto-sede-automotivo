...
export function StoreEventCreateForm({ onSaved }: { onSaved?: () => void }) {
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (error) {
      setMessage('Erro ao cadastrar loja no evento.');
      return;
    }
  ...
  return (
    <form onSubmit={save} className="premium-card p-6">
      ...
      <div className="mt-5 grid gap-3">
        ...
        <input className="premium-input" placeholder="Nome da loja" value={form.storeName} onChange={(e) => setForm({ ...form, storeName: e.target.value })} required />
        <input className="premium-input" placeholder="Nome do responsável" value={form.responsibleName} onChange={(e) => setForm({ ...form, responsibleName: e.target.value })} required />
        <input className="premium-input" placeholder="Telefone" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
        <input className="premium-input" placeholder="E-mail" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required />
      </div>
