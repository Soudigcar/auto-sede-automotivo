...
import { BarChart3, Building2, Clock3, Loader2, Save, ShieldCheck, SlidersHorizontal, Workflow } from 'lucide-react';
...
function Metric({ label }: { label: string }) {
  return <div className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-[9px] font-black uppercase tracking-wide text-zinc-500">{label}</p><p className="mt-1 text-xl font-black text-white">0</p></div>;
}

export function MasterAutocarFollowUpPersistedV2() {
  ...
  const loadInitial = useCallback(async () => {
    ...
    try {
      ...
      const [masterResponse, storesResponse] = await Promise.all([
        fetch('/api/master/autocar/follow-up-v2', { headers: { Authorization: `Bearer ${access}` }, cache: 'no-store' }),
        fetch('/api/master/autocar', { headers: { Authorization: `Bearer ${access}` }, cache: 'no-store' })
      ]);
      const masterBody = await masterResponse.json().catch(() => ({}));
      const storesBody = await storesResponse.json().catch(() => ({}));
      if (!masterResponse.ok) throw new Error(masterBody.error || 'Persistência do Smart Follow-up indisponível.');
      if (!storesResponse.ok) throw new Error(storesBody.error || 'Não foi possível carregar as lojas.');
      setConfig(masterBody.master);
      const rows = (storesBody.stores || []) as StoreRow[];
      setStores(rows);
      setStoreId(rows[0]?.id || '');
      setMessage('Configuração persistida carregada.');
    } catch (error: any) {
      setMessage(error?.message || 'Persistência ainda indisponível neste ambiente.');
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  useEffect(() => {
    ...
    async function loadStore() {
      ...
      try {
        const access = await token();
        if (!access) throw new Error('Sessão Master expirada.');
        const response = await fetch(`/api/master/autocar/follow-up-v2?store_id=${encodeURIComponent(storeId)}`, {
          headers: { Authorization: `Bearer ${access}` }, cache: 'no-store'
        });
        const body = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(body.error || 'Não foi possível carregar o Follow-up da loja.');
        if (!cancelled) {
        ...
        if (!cancelled) {
          setStoreRequested(null);
          setStoreEffective(null);
          setMessage(error?.message || 'Não foi possível carregar o Follow-up da loja.');
        }
      } finally {
  ...
  function toggleScenario(key: FollowUpScenarioKey) {
    setConfig((current) => ({ ...current, scenarios: current.scenarios.map((row) => row.key === key ? { ...row, enabled: !row.enabled } : row) }));
  }
  function changeStep(key: FollowUpScenarioKey, stepId: string, amount: number, unit: DelayUnit) {
    const delayMinutes = (key === 'visit_confirmation' ? -1 : 1) * Math.max(1, amount) * unitMinutes[unit];
    setConfig((current) => ({ ...current, scenarios: current.scenarios.map((row) => row.key === key ? { ...row, steps: row.steps.map((step) => step.id === stepId ? { ...step, delayMinutes } : step) } : row) }));
  }
  function changeAttribution(key: FollowUpScenarioKey, amount: number, unit: DelayUnit) {
    setConfig((current) => ({ ...current, scenarios: current.scenarios.map((row) => row.key === key ? { ...row, attributionWindowMinutes: Math.max(1, amount) * unitMinutes[unit] } : row) }));
  }
  ...
  async function saveMaster() {
    if (!validation.ok || savingMaster) return;
    ...
    try {
      ...
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body.error || 'Não foi possível salvar o teto Master.');
      setConfig(body.master);
      setMessage('Teto Master persistido. AUTOPILOT do Follow-up permanece bloqueado.');
  ...
  async function saveStore() {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(body.error || 'Não foi possível salvar a loja.');
      setStoreRequested(body.store.requested);
      setStoreEffective(body.store.effective);
      setMessage('Configuração da loja persistida em OFF/COPILOT.');
    } catch (error: any) { setMessage(error?.message || 'Falha ao salvar loja.'); }
    finally { setSavingStore(false); }
  }

  const effectivePreview = storeRequested ? clampStoreFollowUpSettings(config.global, storeRequested.global) : storeEffective?.global || null;

  return <div className="mt-6 space-y-5">
    <section className="premium-card p-5 md:p-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div className="flex items-start gap-3"><span className="rounded-xl bg-zinc-950 p-2.5 text-white"><Workflow size={19}/></span><div><h2 className="text-xl font-black">Smart Follow-up V2</h2><p className="mt-1 max-w-3xl text-xs font-bold leading-5 text-zinc-500">Configuração persistida usada pelo Master, Loja e pelo planner da Intelligence V2.</p></div></div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-amber-50 px-3 py-2 text-[10px] font-black uppercase text-amber-800">COPILOT · AUTOPILOT BLOQUEADO</span><button type="button" onClick={() => void saveMaster()} disabled={loading || savingMaster || !validation.ok} className="premium-button-primary"><Save size={14}/>{savingMaster ? 'Salvando...' : 'Salvar Master'}</button></div></div>
      {message ? <div className="mt-4 flex items-center gap-2 rounded-xl border border-zinc-200 bg-zinc-50 p-3 text-xs font-bold text-zinc-700">{loading || loadingStore || savingMaster || savingStore ? <Loader2 size={14} className="animate-spin"/> : null}{message}</div> : null}
    </section>
    ...
    <section className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
      <div className="premium-card p-5"><div className="flex items-center gap-2"><SlidersHorizontal size={17} className="text-red-600"/><h3 className="text-lg font-black">Teto global Master</h3></div><div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <label className="text-xs font-black">Máx. lead/dia<input disabled={loading} type="number" min={1} max={5} value={config.global.maxPerLeadPerDay} onChange={(e) => setGlobal('maxPerLeadPerDay', Number(e.target.value))} className="premium-input mt-1.5"/></label><label className="text-xs font-black">Máx. sequência<input disabled={loading} type="number" min={1} max={10} value={config.global.maxPerSequence} onChange={(e) => setGlobal('maxPerSequence', Number(e.target.value))} className="premium-input mt-1.5"/></label>
