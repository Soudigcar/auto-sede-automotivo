...
export function MasterAutocarAccessControl() {
  ...
  async function save(nextEnabled: boolean, nextAutopilotAllowed: boolean) {
    ...
    try {
      ...
      void body;
      await load();
      setMessage('Governança salva somente na autocar-dev.');
    } catch (error: any) {
  ...
  return (
    <section className="fixed bottom-5 right-5 z-[470] w-[410px] max-w-[calc(100vw-2rem)] rounded-[24px] border border-zinc-200 bg-white p-4 shadow-2xl shadow-black/20">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.16em] text-red-600"><ShieldCheck size={13} /> Governança dupla</p>
      ...
      <p className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-[9px] font-black leading-4 text-amber-800">Preview: mesmo em AUTOPILOT, nenhum envio automático de WhatsApp está habilitado.</p>
