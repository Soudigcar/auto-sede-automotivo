...
export async function POST(request: Request) {
  try {
    ...
    const paymentType = cleanText(body.payment_type, 40) || null;
    if (paymentType && !['cash', 'financed', 'credit_letter'].includes(paymentType)) {
    ...
    if (!payload.has_trade_in) {
      payload.trade_vehicle_configuration_id = null;
      payload.trade_vehicle_name = null;
      payload.trade_vehicle_manufacture_year = null;
      payload.trade_vehicle_model_year = null;
    }
    ...
    await Promise.allSettled([
      context.supabase.from('lead_activity_logs').insert({
        ...
        user_id: context.profile.id, user_name: actorName,
        activity_type: 'commercial_details_updated', activity_label: 'Dados comerciais atualizados',
        from_status: lead.status, to_status: lead.status, customer_name: lead.customer_name,
      ...
      context.supabase.from('audit_logs').insert({
        ...
        new_value: { payment_type: paymentType, has_trade_in: payload.has_trade_in, has_driver_license: payload.has_driver_license, birth_date: payload.birth_date, trade_vehicle_name: payload.trade_vehicle_name }
      })
    ]);

    return NextResponse.json({ success: true, message: 'Dados pessoais e comerciais salvos.', details: data });
  } catch (error: any) {
