...
function extractLead(payload: any) {
  ...
  const phone = digits(nested(data, ['contact.phone','contact.number','contact.whatsapp','customer.phone','sender.phone','sender.number','from','phone','phoneNumber','whatsappNumber','waId']) || findDeep(payload, ['phone','phoneNumber','whatsappNumber','waId','from']));
  const name = nested(data, ['contact.name','contact.fullName','customer.name','sender.name','profile.name','name','contactName','senderName']) || findDeep(payload, ['contactName','senderName','fullName','name']) || phone || 'Lead Umbler Talk';
  const message = nested(data, ['text.body','message.text','message.body','content.text','body','text','message']) || findDeep(payload, ['messageText','body','text','message']);
  const messageId = nested(data, ['message.id','messageId','message_id','id','externalId']) || findDeep(payload, ['messageId','message_id','externalId']);
  const eventName = nested(payload, ['event','eventName','type','name']) || findDeep(payload, ['eventName','event_type']);
  const direction = nested(data, ['direction','message.direction','flow','type']) || findDeep(payload, ['direction','flow']);
  const channelId = nested(data, ['channel.id','channelId','channel_id','instance.id']) || findDeep(payload, ['channelId','channel_id']);
  const chatId = nested(data, ['chat.id','chatId','chat_id','conversation.id','conversationId']) || findDeep(payload, ['chatId','chat_id','conversationId']);
...
function isGroup(payload: any) {
  return Boolean(payload?.isGroup || payload?.data?.isGroup || payload?.chat?.isGroup || payload?.data?.chat?.isGroup);
}
...
async function claimLock(supabase: any, key: string) {
  const { data, error } = await supabase.rpc('claim_lead_ingestion_lock', { p_source: 'umbler_talk', p_dedup_key: key, p_window_seconds: 180 });
  if (error) throw new Error(`Erro na trava anti-duplicidade: ${error.message}`);
  return data !== false;
}

async function pickNextStore(supabase: any, eventId: string) {
  const { data, error } = await supabase.rpc('pick_next_lead_store_by_event', { p_event_id: eventId, p_routing_key: 'umbler_talk' });
  if (error) throw new Error(`Erro ao escolher loja no rodízio do evento: ${error.message}`);
...
async function findExistingLead(supabase: any, phone: string, eventId: string) {
...
async function createLead(supabase: any, lead: ReturnType<typeof extractLead>, payload: any, sourceName: string, event: any) {
  const selectedStore = await pickNextStore(supabase, event.id);
  if (!selectedStore?.store_id) throw new Error(`O evento "${event.event_name}" não possui lojas ativas disponíveis no rodízio.`);

  const assignedAt = new Date().toISOString();
  const { data: routedLead, error } = await supabase.from('leads').insert({
    event_id: event.id,
    ...
    status: 'new_lead',
    notes: ['Lead recebido automaticamente pela Umbler Talk.', `Evento: ${event.event_name}.`, lead.message ? `Primeira mensagem: ${lead.message}` : '', lead.channelId ? `Canal Umbler: ${lead.channelId}` : ''].filter(Boolean).join(' ')
  }).select('id').single();
  if (error) throw new Error(`Erro ao criar lead no pipeline: ${error.message}`);

  const routedLeadId = routedLead?.id || null;
  const metadata = {
    source: 'umbler_talk',
    event_id: event.id,
  ...
  const { data: baseLead, error: baseError } = await supabase.from('leads_base').insert({
    ...
    campaign_id: null, campaign_name: event.event_name,
    vehicle_id: null, vehicle_name: '', vehicle_price: 0, down_payment: 0, financed_amount: 0, installments: 0, estimated_installment: 0, interest_rate: 1.89,
    status: 'Novo lead',
    assigned_store_id: selectedStore.store_id,
    assigned_store_name: selectedStore.store_name,
    assigned_at: assignedAt,
    routed_lead_id: routedLeadId,
    routing_strategy: 'umbler_talk_event_round_robin',
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!lead.phone || lead.phone.length < 10) return NextResponse.json({ ok: true, ignored: 'phone_not_found' });

    const dedupKey = lead.messageId || `${event.id}:${lead.phone}:${lead.message.slice(0, 80)}`;
    if (!(await claimLock(supabase, dedupKey))) return NextResponse.json({ ok: true, ignored: 'duplicate_event' });

    const sourceName = cleanText(integration.settings.source_name) || defaultSettings.source_name;
    const existing = await findExistingLead(supabase, lead.phone, event.id);
    const result = existing ? await appendExistingLead(supabase, existing, lead, payload) : await createLead(supabase, lead, payload, sourceName, event);

    await updateIntegrationStatus(supabase, integration, { last_webhook_at: new Date().toISOString(), last_error: '', last_lead_phone: lead.phone, last_lead_id: result.id || '', last_event_id: event.id, last_event_name: event.event_name });
