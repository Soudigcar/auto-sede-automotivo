...
async function loadSnapshot(supabase: any) {
  ...
  const activeStoreIds = new Set<string>(stores.filter((store: any) => store.status === 'active' && store.portal_enabled === true).map((store: any) => String(store.id)));
  const candidates = (vehiclesResult.data || []).filter((vehicle: any) => vehicle.status === 'disponivel' && vehicle.show_on_landing === true && Number(vehicle.price || 0) > 0);
  ...
  const { data: linkRows, error: linkError } = legacyIds.length
    ? await supabase.from('store_vehicle_link_submissions').select('imported_vehicle_id,store_id,status,metadata').in('imported_vehicle_id', legacyIds)
    : { data: [], error: null };
  ...
  const publicVehicles = candidates.filter((vehicle: any) => {
    ...
    return owners.length === 1 && Boolean(owners[0]) && activeStoreIds.has(owners[0]);
  }).length;
...
async function loadSettings(supabase: any) {
  const { data, error } = await supabase.from('portal_settings').select('*').eq('key', 'official').maybeSingle();
  if (error) {
...
export async function GET(request: Request) {
  try {
    ...
    const [settingsResult, snapshot] = await Promise.all([loadSettings(context.supabase), loadSnapshot(context.supabase)]);
    return NextResponse.json({ cms_ready: settingsResult.cmsReady, settings: settingsResult.settings, snapshot });
  } catch (error: any) {
...
export async function PUT(request: Request) {
  try {
    ...
    const { data, error } = await context.supabase.rpc('save_portal_settings_transaction', { p_actor_user_id: context.profile.id, p_settings: settings });
    if (error) {
    ...
    return NextResponse.json({ success: true, message: settings.is_published ? 'Configuração salva e publicada no portal.' : 'Configuração salva como rascunho.', settings: normalizePortalSettings(data || settings) });
  } catch (error: any) {
