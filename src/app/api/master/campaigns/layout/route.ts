...
export async function POST(request: Request) {
  try {
    ...
    const { data: savedLayout, error: layoutError } = await supabase.from('site_campaign_layouts').upsert(layoutPayload, { onConflict: 'campaign_id' }).select('campaign_id,editor_draft,published_layout,layout_version,draft_updated_at,published_at,published_by').single();
    if (layoutError || !savedLayout) return NextResponse.json({ error: layoutError?.message || 'Não foi possível salvar o layout.' }, { status: 400 });

    let campaignState = campaign;
    if (action === 'publish') {
      const { data: publishedCampaign, error: publishError } = await supabase.from('site_campaigns').update({ is_active: true, published_at: now, updated_at: now }).eq('id', id).select('id,slug,is_active,published_at').single();
      if (publishError || !publishedCampaign) return NextResponse.json({ error: publishError?.message || 'O layout foi salvo, mas a landing não pôde ser ativada.' }, { status: 400 });
