...
export default function MasterPrivateLeadTransferPage() {
  ...
  async function load() {
    const [leadResult, storeResult] = await Promise.all([
      supabase
        .from('leads_base')
        .select('id,name,phone,source,status,event_id,assigned_store_id,assigned_store_name')
  ...
  useEffect(() => {
    load().catch(() => setMessage('Não foi possível carregar a Base Master.'));
          ...
          <section className="grid gap-4 lg:grid-cols-[1fr_340px]">
            <div className="rounded-3xl border border-zinc-200 bg-white p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <button type="button" onClick={chooseAll} className="rounded-xl bg-slate-950 px-4 py-2 text-[10px] font-black uppercase text-white">Selecionar todos visíveis</button>
              </div>
