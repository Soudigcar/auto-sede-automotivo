    ...
    <main className="premium-page">
      <section className="premium-shell flex min-h-screen">
        ...
        <div className="premium-canvas min-w-0 flex-1 p-4 md:p-7">
          <header className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between"><div><p className="premium-eyebrow">Gestao Master</p><h1 className="premium-title mt-2 text-4xl md:text-5xl">Lojas Participantes</h1><p className="premium-muted mt-3 max-w-3xl text-sm">Cadastro e gestao das lojas participantes do evento.</p></div><Link href="/master/dashboard/live" className="premium-button-secondary"><BarChart3 size={18} /> Voltar ao Dashboard</Link></header>
          ...
          <section className="mt-7 grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
            <form onSubmit={handleSubmit} className="premium-card p-6"><h2 className="text-2xl font-black text-zinc-950">Cadastrar loja</h2><div className="mt-5 grid gap-3"><input className="premium-input" placeholder="Nome da loja" value={form.storeName} onChange={(event) => setForm({ ...form, storeName: event.target.value })} required /><input className="premium-input" placeholder="Nome do responsavel" value={form.responsibleName} onChange={(event) => setForm({ ...form, responsibleName: event.target.value })} required /><input className="premium-input" placeholder="Telefone do responsavel" value={form.responsiblePhone} onChange={(event) => setForm({ ...form, responsiblePhone: event.target.value })} /><input className="premium-input" placeholder="E-mail do responsavel" value={form.responsibleEmail} onChange={(event) => setForm({ ...form, responsibleEmail: event.target.value })} /><button className="premium-button-primary" type="submit">Cadastrar loja</button></div></form>
            <div className="premium-card p-6"><h2 className="text-2xl font-black text-zinc-950">Lojas cadastradas</h2><div className="mt-5 grid gap-3">{stores.map((store) => <div key={store.id} className="rounded-2xl border border-zinc-100 bg-zinc-50 p-4"><h3 className="font-black text-zinc-950">{store.store_name}</h3><p className="mt-1 text-sm text-zinc-500">Responsavel: {store.responsible_name}</p><p className="mt-1 text-xs text-zinc-400">{store.responsible_phone || 'Telefone nao informado'} | {store.responsible_email || 'E-mail nao informado'}</p></div>)}{stores.length === 0 ? <p className="text-sm text-zinc-500">Nenhuma loja cadastrada.</p> : null}</div></div>
          </section>
        </div>
      </section>
    </main>
