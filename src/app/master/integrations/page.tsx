...
export default function MasterIntegrationsPage() {
  ...
  async function savePixel(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();

    setSaving(true);
    setMessage('Salvando Pixel...');
    ...
    try {
      ...
      setMessage('Pixel salvo com sucesso. A landing já pode disparar eventos.');
      await loadPixel();
    } catch {
      setMessage('Erro ao salvar Pixel.');
    }
        ...
        <div className="premium-canvas min-w-0 flex-1 p-4 md:p-7">
          <header className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="premium-muted mt-3 max-w-3xl text-sm">
                Central de conexões técnicas do sistema. Começamos pelo Pixel do Facebook/Meta para rastrear landing, simulador e leads.
              </p>
            </div>
          ...
          <form onSubmit={savePixel} className="mt-7 grid gap-5 xl:grid-cols-[1fr_420px]">
            <section className="premium-card p-6">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm font-bold text-zinc-500">
                    Cole o ID do Pixel para instalar na landing pública e medir os eventos do simulador.
                  </p>
                </div>
              ...
              <div className="mt-6 grid gap-4">
                ...
                <label className="grid gap-2">
                  <span className="text-xs font-black uppercase tracking-wide text-zinc-500">ID do Pixel</span>
                  <input
                    className="premium-input"
                    value={form.pixel_id}
                ...
                <label className="flex items-center justify-between gap-4 rounded-[24px] border border-zinc-100 bg-zinc-50 p-4">
                  <div>
                    <p className="text-sm font-black text-zinc-950">Ativar Pixel na landing</p>
                    <p className="mt-1 text-xs font-bold text-zinc-500">
                      Quando ativo, o script será carregado no site público e no simulador.
                    </p>
                  </div>
            ...
            <section className="premium-card p-6">
              ...
              <p className="mt-2 text-sm font-bold text-zinc-500">
                O principal evento para campanhas será Lead.
              </p>
              ...
              <div className="mt-5 grid gap-3">
                <button className="premium-button-primary justify-center" type="submit" disabled={saving || loading}>
                  <Save size={18} /> {saving ? 'Salvando...' : 'Salvar Pixel'}
                </button>
