...
function LeadCard({ lead, columnKey, tone, dragging, onDragStart, onDragEnd, onOpen, onReveal, onStart, onSchedule, onMove, onCancel, onSale, onLost, onReopen, onTask, onTransfer }: {
  ...
  return (
    <div data-lead-id={lead.id} role="button" tabIndex={0} draggable onClick={onOpen} onKeyDown={(event) => event.key === 'Enter' && onOpen()} onDragStart={onDragStart} onDragEnd={onDragEnd} className={`cursor-pointer overflow-hidden rounded-2xl border border-zinc-100 bg-white p-2.5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${dragging ? 'opacity-50 ring-2 ring-red-300' : ''}`}>
      <div className="flex items-start justify-between gap-2"><div className="min-w-0 flex-1"><h3 className="truncate text-[12px] font-black leading-tight text-zinc-950">{lead.customer_name || 'Cliente sem nome'}</h3><p className="mt-0.5 truncate text-[9px] font-bold leading-tight text-zinc-500">{lead.interested_vehicle || 'Interesse não informado'}</p></div><span className={`rounded-full px-2 py-1 text-[9px] font-black uppercase ${styles.badge}`}>{statusLabels[lead.status] || lead.status}</span></div>
      <div className="mt-1.5 flex min-w-0 flex-nowrap items-center gap-1 overflow-hidden"><button type="button" title="Visualizar telefone" onClick={(event) => stop(event, onReveal)} className="min-w-0 max-w-[42%] flex-[0_1_42%] truncate rounded-full bg-zinc-50 px-1.5 py-1 text-[8px] font-black leading-none text-zinc-500"><Eye size={10} className="mr-1 inline shrink-0" />{phone}</button><span className="min-w-0 flex-1 truncate rounded-full bg-zinc-50 px-1.5 py-1 text-[8px] font-black uppercase leading-none text-zinc-500">{readableOrigin(lead.origin)}</span><span className="shrink-0 rounded-full bg-zinc-900 px-1.5 py-1 text-[8px] font-black uppercase leading-none text-white">{formatLeadAge(lead.created_at)}</span></div>
      {lead.scheduled_at ? <div className="mt-1.5 rounded-lg bg-zinc-50 p-1.5 text-[9px] leading-tight text-zinc-600"><p className="flex items-center gap-1 font-black text-zinc-900"><CalendarClock size={11} /> {formatDateTime(lead.scheduled_at)}</p>{lead.appointment_notes ? <p className="mt-1 line-clamp-2">{lead.appointment_notes}</p> : null}</div> : null}
      {columnKey === 'appointment_cancelled' ? <div className="mt-1.5 rounded-lg bg-orange-50 p-1.5 text-[9px] leading-tight text-orange-800"><p className="font-black">Cancelado {formatDateTime(lead.appointment_cancelled_at)}</p><p className="mt-1 line-clamp-2">{lead.appointment_cancelled_reason}</p></div> : null}
      {columnKey === 'lost' ? <div className="mt-1.5 rounded-lg bg-red-50 p-1.5 text-[9px] leading-tight text-red-700"><p className="font-black">Motivo da perda</p><p className="mt-1 line-clamp-2">{lead.lost_reason || 'Perda registrada'}</p></div> : null}
      <div className="mt-2 flex w-full flex-nowrap items-center gap-1 overflow-hidden">
        ...
        {columnKey === 'new_lead' ? <><SmallAction label={lead.has_phone ? 'WhatsApp' : 'Atender'} tone="green" icon={<MessageCircle size={12} />} onClick={(event) => stop(event, onStart)} /><SmallAction label="Perda" tone="red" icon={<XCircle size={12} />} onClick={(event) => stop(event, onLost)} /></> : null}
        {columnKey === 'in_service' ? <><SmallAction label="Agendar" tone="red" icon={<CalendarDays size={12} />} onClick={(event) => stop(event, onSchedule)} /><SmallAction label="Perda" tone="red" icon={<XCircle size={12} />} onClick={(event) => stop(event, onLost)} /></> : null}
        {columnKey === 'scheduled' ? <><SmallAction label="Chegou" tone="blue" icon={<CheckCircle2 size={12} />} onClick={(event) => stop(event, () => onMove('showed_up'))} /><SmallAction label="Reagendar" icon={<RotateCcw size={12} />} onClick={(event) => stop(event, onSchedule)} /><SmallAction label="Cancelou" tone="orange" icon={<XCircle size={12} />} onClick={(event) => stop(event, onCancel)} /><SmallAction label="Faltou" icon={<XCircle size={12} />} onClick={(event) => stop(event, () => onMove('no_show'))} /></> : null}
        {columnKey === 'appointment_cancelled' || columnKey === 'no_show' ? <><SmallAction label="Reagendar" tone="red" icon={<RotateCcw size={12} />} onClick={(event) => stop(event, onSchedule)} /><SmallAction label="Perda" tone="red" icon={<XCircle size={12} />} onClick={(event) => stop(event, onLost)} /></> : null}
        {columnKey === 'showed_up' ? <><SmallAction label="Venda" tone="green" icon={<CheckCircle2 size={12} />} onClick={(event) => stop(event, onSale)} /><SmallAction label="Perda" tone="red" icon={<XCircle size={12} />} onClick={(event) => stop(event, onLost)} /></> : null}
...
function SmallAction({ label, onClick, icon, tone = 'default' }: { label: string; onClick: (event: any) => void; icon: ReactNode; tone?: 'default' | 'green' | 'red' | 'orange' | 'blue' }) {
  ...
  return <button className={`inline-flex h-6 w-6 min-w-6 flex-none items-center justify-center rounded-md border p-0 ${className}`} type="button" title={label} aria-label={label} onClick={onClick}>{icon}<span className="sr-only">{label}</span></button>;
}
