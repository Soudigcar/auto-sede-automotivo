...
export default function StoreAutocarFollowUpPage() {
  ...
  return <main className="premium-page"><div className="premium-canvas min-w-0 p-4 md:p-7"><header><Link href={`/loja/${portal.store.slug}/autocar`} prefetch={false} className="premium-button-secondary mb-4 inline-flex"><ArrowLeft size={15}/>Voltar para AUTOCAR</Link><div className="premium-eyebrow text-red-600">I.A AUTOCAR · SMART FOLLOW-UP</div><h1 className="premium-title mt-2 text-4xl md:text-5xl">Follow-up da Loja</h1><p className="premium-muted mt-3 max-w-4xl text-sm">Configure jornadas e limites da sua loja dentro do teto definido pelo Master. O COPILOT pode preparar rascunhos para revisão humana, sem envio automático.</p></header>{loading ? <div className="mt-6 flex items-center gap-3 rounded-2xl border border-zinc-200 bg-white p-4 text-sm font-bold text-zinc-600"><Loader2 size={18} className="animate-spin text-red-600"/>Validando permissões da AUTOCAR...</div> : null}{message ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-bold text-amber-900">{message}</div> : null}{!loading ? <div className="mt-6 space-y-6"><StoreAutocarFollowUpV2 storeName={portal.store.store_name} canManage={canManage}/><StoreAutocarFollowUpCopilotQueue slug={portal.store.slug}/></div> : null}</div></main>;
}
