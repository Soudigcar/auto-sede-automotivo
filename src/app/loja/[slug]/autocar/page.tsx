...
import { AutocarOperationalProfile } from '@/components/AutocarOperationalProfile';
      ...
      <div className="premium-canvas min-w-0 p-4 md:p-7">
        <header className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <p className="premium-muted mt-3 max-w-3xl text-sm">Central para ensinar a AUTOCAR a vender pelo Método Venda Mais, respeitando o contexto, as regras e o conhecimento exclusivo de cada loja.</p>
          </div>
        ...
        {message ? <div className="mt-6 flex items-center gap-3 rounded-2xl border border-zinc-200 bg-white p-4 text-sm font-bold text-zinc-600">{!status ? <Loader2 size={18} className="animate-spin text-red-600" /> : null}{message}</div> : null}

        <AutocarIntelligenceCenter storeName={status?.store_scope.store_name || portal.store.store_name} slug={portal.store.slug} canManage={Boolean(status?.permissions.manage)} />
        <AutocarOperationalProfile slug={portal.store.slug} canManage={Boolean(status?.permissions.manage)} />
        ...
        <section className="mt-6 grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
          <div className="premium-card p-5 md:p-6">
            <div className="flex items-start justify-between gap-4"><div><p className="premium-eyebrow">Isolamento multi-loja</p><h2 className="mt-2 flex items-center gap-2 text-xl font-black text-zinc-950"><LockKeyhole size={20} className="text-red-600" /> Loja definida pelo backend</h2></div><span className="rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-black uppercase text-emerald-700">Obrigatório</span></div>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-3"><SecurityPoint title="Contexto" text="Conversa, integração, lead e mensagem são validados contra a mesma loja." /><SecurityPoint title="Venda Mais" text="Método oficial herdado; a loja adiciona contexto sem remover regras essenciais." /><SecurityPoint title="Operação" text="Horário, localização e agenda usam fontes estruturadas; dados ausentes não são inventados." /></div>
          </div>

          <div className="premium-card p-5 md:p-6">
            <p className="premium-eyebrow">Permissões</p><h2 className="mt-2 text-xl font-black text-zinc-950">Acesso desta sessão</h2>
            <div className="mt-5 space-y-3"><PermissionRow label="Visualizar AUTOCAR" enabled={Boolean(status?.permissions.view)} /><PermissionRow label="Gerenciar AUTOCAR" enabled={Boolean(status?.permissions.manage)} /><PermissionRow label="Aprovar ações críticas" enabled={Boolean(status?.permissions.approve)} /></div>
          </div>
        </section>
...
function StatusCard({ icon, label, value, helper, ok }: { icon: React.ReactNode; label: string; value: string; helper: string; ok: boolean }) { return <div className="premium-card p-5"><div className={`flex h-10 w-10 items-center justify-center rounded-xl ${ok ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>{icon}</div><p className="mt-4 text-[10px] font-black uppercase tracking-[0.14em] text-zinc-400">{label}</p><p className="mt-1 text-lg font-black text-zinc-950">{value}</p><p className="mt-2 text-xs leading-5 text-zinc-500">{helper}</p></div>; }
function SecurityPoint({ title, text }: { title: string; text: string }) { return <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4"><p className="text-xs font-black text-zinc-900">{title}</p><p className="mt-2 text-xs leading-5 text-zinc-500">{text}</p></div>; }
function PermissionRow({ label, enabled }: { label: string; enabled: boolean }) { return <div className="flex items-center justify-between gap-3 rounded-xl border border-zinc-200 px-3 py-3"><span className="text-sm font-bold text-zinc-700">{label}</span>{enabled ? <CheckCircle2 size={18} className="text-emerald-600" /> : <XCircle size={18} className="text-zinc-300" />}</div>; }
