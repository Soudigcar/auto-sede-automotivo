...
function LoginContent() {
  ...
  async function handleLogin(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (profile.role === 'master') {
      router.push(redirectedFrom?.startsWith('/master') ? redirectedFrom : '/master/dashboard/live');
      router.refresh();
      return;
    }
      ...
      <section className="mx-auto flex min-h-[calc(100vh-64px)] max-w-5xl items-center justify-center">
        <div className="grid w-full gap-6 lg:grid-cols-[1fr_430px] lg:items-center">
          ...
          <form onSubmit={handleLogin} className="rounded-[34px] border border-white/10 bg-white p-7 text-[#101828] shadow-2xl shadow-black/40 md:p-8">
            ...
            <input className="mt-2 w-full rounded-xl border border-zinc-800 bg-[#111827] px-4 py-3 font-semibold text-white outline-none focus:border-red-500" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="seuemail@dominio.com.br" autoComplete="email" />

            <label className="mt-4 block text-sm font-bold text-zinc-700">Senha</label>
            <input className="mt-2 w-full rounded-xl border border-zinc-800 bg-[#111827] px-4 py-3 font-semibold text-white outline-none focus:border-red-500" type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Digite sua senha" autoComplete="current-password" />

            <button className="btn-primary mt-6 w-full justify-center" type="submit">Entrar no sistema</button>
