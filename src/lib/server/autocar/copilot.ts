...
export async function analyzeAutocarCopilot(source: AutocarCopilotSource) {
  ...
  const instructions = [
    ...
    'O campo store_inventory é a fonte oficial e em tempo real do estoque interno da loja desta conversa. O backend já determinou a loja correta; nunca peça, escolha ou altere store_id.',
    'Interprete linguagem natural, abreviações, erros de digitação e variações de pontuação do cliente. Exemplos como HRV, HR-V e HR V podem representar o mesmo modelo quando isso for semanticamente plausível.',
    ...
    'Extraia apenas fatos explícitos ou inferências comerciais conservadoras apoiadas pela conversa e pelos dados reais fornecidos.',
    'Quando um campo não estiver claro, retorne null. Nunca invente aprovação de crédito, desconto, avaliação de troca ou condição comercial.',
