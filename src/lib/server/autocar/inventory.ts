...
export type AutocarInventoryContext = {
  ...
  inventory_index: Array<Pick<AutocarInventoryVehicle, 'id' | 'brand' | 'model' | 'version' | 'year' | 'transmission' | 'fuel' | 'price'>>;
  constraints: {
...
function scoreVehicle(row: any, normalizedQuery: string, tokens: string[]) {
  ...
  tokens.forEach((token) => { if (haystack.includes(token)) score += 18; });
  return score;
}
...
function toVehicle(row: any, portalEnabled: boolean, score: number): AutocarInventoryVehicle {
  ...
  if (published && row?.id) {
    portalUrl = absolutePortalUrl(publicVehiclePath({ id: String(row.id), brand, model, year } as any));
  }
...
export async function loadAutocarInventory(input: {
  ...
  [...(directRows || []), ...legacyRows].forEach((row: any) => {
  ...
  const scored = rows
    ...
    .map((row) => ({ row, score: scoreVehicle(row, normalizedQuery, tokens) }))
  ...
  const inventoryIndex = rows
    .slice(0, indexLimit)
    .map((row) => toVehicle(row, portalEnabled, 0))
    .map(({ id, brand, model, version, year, transmission, fuel, price }) => ({ id, brand, model, version, year, transmission, fuel, price }));
