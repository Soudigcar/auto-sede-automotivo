...
const LIVE_PILOT_VERSION = 'autocar-live-location-a4-v1';
...
async function currentLiveEligibility(storeId: string, conversationId: string, operationalPreview: any) {
  ...
  return { allowed: true, reason: 'Elegível para LIVE LOCATION V1.', runtime, policy };
}
...
async function createLocationClaim(input: {
  ...
  const { data, error } = await autocar.from('ai_runtime_message_claims').insert({
    ...
    policy_reason: input.gateReason || 'A4 LIVE LOCATION V1: endereço e Google Maps reais liberados após guard operacional.',
    result: {
...
export async function attemptAutocarLiveLocationPilot(input: {
  ...
  if (process.env.VERCEL_ENV !== 'preview') {
    return { sent: false, skipped: true, reason: 'LIVE LOCATION V1 é bloqueado fora de Preview.' };
  }
  if (input.storeId !== A4_PILOT_STORE_ID) {
    return { sent: false, skipped: true, reason: 'LIVE LOCATION V1 está liberado somente para A4 Multimarcas.' };
  }
