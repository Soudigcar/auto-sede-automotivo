...
export async function reviewVehicleImportWithOpenAI(input: VehicleImportInput, sourceLabel = 'site público da loja'): Promise<AiReviewResult> {
  ...
  try {
    ...
    const reviewedVehicle = safeVehicle({ ...parsed?.vehicle, source_url: vehicle.source_url });
    return {
      ok: true,
      model: payload?.model || model,
      vehicle: reviewedVehicle,
      optimized_description: ensureMileageInDescription(cleanText(parsed?.optimized_description, 12000), reviewedVehicle.mileage),
...
export function mergeImportedVehicle(baseInput: VehicleImportInput, reviewedInput: VehicleImportInput) {
  ...
  const mileage = reviewed.mileage || base.mileage;
  return {
